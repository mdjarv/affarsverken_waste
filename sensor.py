                ...
                vol.Schema(
                    {
                        ...
                        vol.Optional(
                            CONF_NAME
                        ): cv.string,  # Name for this specific address entry
                    }
                )
            ],
        ),
...
def _load_cache_data(hass: HomeAssistant) -> dict:
    ...
    try:
        ...
        _LOGGER.warning(
            f"Error loading cache file '{cache_file_path}': {e}. Cache will be ignored."
        )
        return {}
...
def _save_cache_data(hass: HomeAssistant, data: dict):
    ...
    try:
        ...
        _LOGGER.error(
            f"Error saving cache file '{cache_file_path}': {e}. Cache will not be persisted."
        )
...
class AffarsverkenWasteApiClient:
    ...
    def _get_auth_token(self) -> str | None:
        ...
        if (
            ...
            and datetime.now(UTC) < token_expiration_time
        ):
        ...
        _LOGGER.info(
            "Cached token is expired or not available. Attempting to get a new authentication token..."
        )
        try:
            ...
            try:
                ...
                _LOGGER.error(
                    f"Raw response content on error: {e.response.text if e.response else 'No response content'}"
                )
            return None
        ...
        if not token:
            _LOGGER.error(
                "Login successful, but no token found in plain text response."
            )
        ...
        try:
            ...
            if "exp" in decoded_token:
                new_token_expiration_time = datetime.fromtimestamp(
                    decoded_token["exp"], tz=UTC
                )
                new_token_expiration_time -= timedelta(minutes=5)
                _LOGGER.info(
                    f"New token obtained. Expires UTC: {new_token_expiration_time.strftime('%Y-%m-%d %H:%M:%S')}"
                )
            else:
                new_token_expiration_time = datetime.now(UTC) + timedelta(hours=1)
                _LOGGER.warning(
                    f"New token obtained, but 'exp' claim not found. Assuming 1 hour validity. Expires UTC: {new_token_expiration_time.strftime('%Y-%m-%d %H:%M:%S')}"
                )

        except jwt.DecodeError as e:
            _LOGGER.warning(
                f"Could not decode JWT to get expiration time: {e}. Assuming 1 hour validity."
            )
            new_token_expiration_time = datetime.now(UTC) + timedelta(hours=1)
        except Exception as e:
            _LOGGER.warning(
                f"Unexpected error when processing token: {e}. Assuming 1 hour validity."
            )
            new_token_expiration_time = datetime.now(UTC) + timedelta(hours=1)
    ...
    def search_building_id(self, address: str) -> str | None:
        ...
        if cached_building_info:
            ...
            if last_updated_str and query_param:
                ...
                if datetime.now(UTC) - last_updated < BUILDING_CACHE_LIFETIME:
                    _LOGGER.debug(
                        f"Using cached building info for address '{address}'."
                    )
                    return query_param

        _LOGGER.info(
            f"Cached building info for '{address}' is expired or not available. Attempting to search..."
        )
        ...
        try:
            ...
            if not search_results or not isinstance(search_results, list):
                _LOGGER.warning(
                    "Building search returned no results or unexpected format."
                )
                return None
            ...
            if not query_param:
                _LOGGER.warning(
                    f"'query' parameter not found in search result: {first_result}"
                )
                return None
            ...
            _LOGGER.debug(
                f"Successfully found and cached building query parameter for '{address}': {query_param}"
            )
            return query_param
    ...
    def get_waste_data(self, address: str) -> dict | None:
        ...
        if not query_param:
            _LOGGER.error(
                f"Could not get building query parameter for address: {address}"
            )
            return None
    ...
    def parse_collection_dates(self, data: dict) -> dict:
        ...
        if not isinstance(data.get("services"), list):
            _LOGGER.warning(
                "Warning: 'services' key not found or is not a list in the response data."
            )
        ...
        for service in data["services"]:
            ...
            if not (title and next_pickup_str):
                _LOGGER.warning(
                    f"Missing 'title' or 'nextPickup' in service item: {service}. Skipping."
                )
                continue
            ...
            try:
                ...
                _LOGGER.warning(
                    f"Could not parse date '{next_pickup_str}' for type '{title}'. Skipping."
                )
        return collection_dates
...
async def async_setup_entry(
    ...
    async def async_update_data_for_address(target_address: str = address):
        ...
        try:
            waste_data = await hass.async_add_executor_job(
                api_client.get_waste_data, target_address
            )
            if not waste_data:
            ...
            if not parsed_dates:
                raise UpdateFailed(
                    f"No waste collection dates parsed from data for {target_address}."
                )

            return {"parsed_dates": parsed_dates, "raw_data": waste_data}
        except Exception as err:
            raise UpdateFailed(
                f"Error communicating with API for {target_address}: {err}"
            ) from err
...
class AffarsverkenWasteSensor(SensorEntity):
    ...
    def __init__(
        ...
        address_hash = hashlib.md5(self._address.encode()).hexdigest()[:8]
        self._attr_unique_id = f"{DOMAIN}_{address_hash}_{slugify(self._waste_type)}"
        ...
        self._attr_icon = (
            "mdi:trash-can"  # Fallback icon if device class doesn't provide one
        )
            ...
            attributes.update(
                {
                    ...
                    "pickup_date": collection_date.strftime("%Y-%m-%d"),
                    "waste_type": self._waste_type,
                    "address": self._address,
                    "is_today": days_until_pickup == 0,
                    "is_tomorrow": days_until_pickup == 1,
                    "is_this_week": 0 <= days_until_pickup <= 7,
    ...
    def device_info(self):
        ...
        waste_types = list(self.coordinator.data["parsed_dates"].keys())
        return {
            "identifiers": {(DOMAIN, self._address)},
            "name": f"Waste Collection - {self._address}",
            "manufacturer": "Affärsverken",
            "model": f"{len(waste_types)} Collection Types",
